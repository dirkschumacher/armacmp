...
The package supports only a very limited number of types. For scalar values these are `bool`, `int` and `double`. All matrices and vectors are of type `arma::mat/colvec/rowvec` for double matrices, and `arma::imat/icolvec/irowvec` for integer matrices. By default, it is assumed that a variable is of type `arma::mat` if nothing else is specified/can be deduced.
